...
define(`TRISO_USE_PCINT_STROBE_PRES', `dnl
/* RFID PinChange-Interrupt Line  PCINT$1 for Strobe and PCINT$2 for tagPresence */
/* alez@maetech.it */
dnl Configure pin-change-mask to monitor PCINTn and enable interrupt
#define triso_configure_pcint() \
  _paste(PCMSK, eval($1/8)) |= _BV(PCINT$1); \
  PCICR  |= _BV(_paste(PCIE, eval($1/8))); \
  _paste(PCMSK, eval($2/8)) |= _BV(PCINT$2); \
  PCICR  |= _BV(_paste(PCIE, eval($2/8)));

#define BADGE_VECTOR _paste3(PCINT, eval($1/8), _vect)
')

define(`KDR1000_USE_PCINT_STROBE_PRES', `dnl
/* MSR PinChange-Interrupt Line  PCINT$1 for Strobe and PCINT$2 for tagPresence */
/* alez@maetech.it */
dnl Configure pin-change-mask to monitor PCINTn and enable interrupt
#define kdr1000_configure_pcint() \
  _paste(PCMSK, eval($1/8)) |= _BV(PCINT$1); \
  PCICR  |= _BV(_paste(PCIE, eval($1/8))); \
  _paste(PCMSK, eval($2/8)) |= _BV(PCINT$2); \
  PCICR  |= _BV(_paste(PCIE, eval($2/8)));
